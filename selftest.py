...
async def main() -> int:
    ...
    _sh = (
        "#!/bin/bash\n"
        "# Add two integers via env.\n"
        "# @param {integer} a\n"
        "# @param {integer} b\n"
        "echo $(( SLEUTH_ARG_A + SLEUTH_ARG_B ))\n"
    )
    _sh_msg = _skills.skill_write("add_sh", _sh, language="bash")
    check("skill_write authors a bash skill",
          "live and callable" in _sh_msg and "bash" in _sh_msg.lower(), _sh_msg[:200])
    _sh_h = _agent.resolve_handler("add_sh")
